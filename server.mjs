...
const DATA = process.env.DREAMAPI_DATA_DIR || join(ROOT, 'data');
const TASK_FILE = join(DATA, 'tasks.json');
const OUTPUTS = join(DATA, 'outputs');
const BASE_URL = process.env.DREAMAPI_BASE_URL || 'https://api.newportai.com';
const ENV_KEY = process.env.DREAMAPI_API_KEY?.trim() || '';
const MOCK = process.env.DREAMAPI_MOCK === 'true';
const PORT = Number(process.env.PORT || 8788);
const DEPLOYMENT_MODE = process.env.DREAMAPI_DEPLOYMENT_MODE === 'hosted' ? 'hosted' : 'local';
